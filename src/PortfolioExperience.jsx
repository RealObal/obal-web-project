...
import {ImportedSections} from './PortfolioSections.jsx';
...
function ContactPanel(){
  ...
  async function submit(e){
    ...
    if(result.ok)setForm({name:'',email:'',phone:'',message:''});
  }
  return <div className="contact-panel" id="contact"><div className="contact-intro"><span className="eyebrow">LET’S TURN EVIDENCE INTO ACTION</span><h2>Good work starts<br/>with a <em>conversation.</em></h2><p>Exploring a research collaboration, an M&E role, or a programme learning challenge? Send a note or connect directly.</p><div className="contact-direct"><a href="mailto:ronaldobal20@gmail.com"><Mail size={16}/>ronaldobal20@gmail.com</a><a href="tel:+256776280494">+256 776 280 494</a><a href="https://linkedin.com/in/ronaldobal" target="_blank" rel="noopener noreferrer"><Linkedin size={16}/>LinkedIn<ExternalLink size={14}/></a></div></div><form className="contact-form" onSubmit={submit}><label>Name<input required name="name" autoComplete="name" value={form.name} onChange={update}/></label><label>Email<input required type="email" name="email" autoComplete="email" value={form.email} onChange={update}/></label><label>Phone <span>(optional)</span><input type="tel" name="phone" autoComplete="tel" value={form.phone} onChange={update}/></label><label>Message<textarea required name="message" rows="5" value={form.message} onChange={update}/></label><button className="button cream" type="submit" disabled={state.sending}>{state.sending?'Sending…':'Send message'}<ArrowUpRight size={18}/></button>{state.message&&<p className={`form-status ${state.kind}`} role="status">{state.message}</p>}</form></div>;
}
...
export default function App({initialPath='/'}){
  ...
  const [profileOpen,setProfileOpen]=useState(initialPath==='/about/'||initialPath==='/about');
  ...
  function openItem(e,item){if(e.metaKey||e.ctrlKey||e.shiftKey||e.altKey)return;e.preventDefault();window.history.pushState(null,'',pathFor(item)+window.location.search);setSelected(item);}
  ...
  const filtered=work.filter(item=>(type==='All work'||item.type===type)&&(skill==='All tools'||item.skills.includes(skill))&&`${item.title} ${item.fullTitle} ${item.summary} ${item.theme} ${item.skills.join(' ')}`.toLowerCase().includes(query.toLowerCase().trim())).sort((a,b)=>sort==='title'?a.title.localeCompare(b.title):Number(!!b.featured)-Number(!!a.featured));
  ...
  const workSchema=selected?{'@context':'https://schema.org','@type':selected.type==='Publication'?'ScholarlyArticle':'CreativeWork',name:selected.fullTitle,description:selected.summary,url:`https://ronaldobal.com${pagePath}`,author:{'@type':'Person',name:'Ronald Obal',url:'https://ronaldobal.com/about'},sameAs:selected.source||undefined}:undefined;
  return <>
    <Seo title={pageTitle} description={pageDescription} path={pagePath} type={selected?.type==='Publication'?'article':'profile'} image="https://ronaldobal.com/ronald-portrait.webp" jsonLd={selected?[workSchema,breadcrumbSchema([{name:'Ronald Obal',path:'/'},{name:'Selected work',path:'/#work'},{name:selected.fullTitle,path:pagePath}])]:[personSchema,websiteSchema,profilePageSchema,siteNavigationSchema]}/>
    <a className="skip-link" href="#main">Skip to content</a>
    <header className="site-header" inert={selected?true:undefined}><div className="container header-inner"><a href="/" className="brand" aria-label="Ronald Obal home">ro<span>.</span></a><nav aria-label="Main navigation"><a href="#work">Selected work</a><a href="#toolkit">Toolkit</a><a href="/blog/">Insights</a><a href="#profile" onClick={()=>setProfileOpen(true)}>About</a></nav><a className="header-contact" href="#contact">Let’s connect<ArrowUpRight size={17}/></a></div></header>
    <main id="main" inert={selected?true:undefined}>
      ...
      <section id="work" className="work-section" aria-labelledby="work-title"><div className="container"><div className="section-heading"><div><span className="eyebrow">01 / SELECTED WORK</span><h2 id="work-title">From questions<br/>to <em>understanding.</em></h2></div><p>A closer look at my research, programme experience, and projects. Find the work that connects with your interests.</p></div>
        ...
        <div className="work-grid">{filtered.map(item=><article className="work-card" key={item.id}><a className="visual-link" href={pathFor(item)} onClick={e=>openItem(e,item)} tabIndex={-1} aria-hidden="true"><Visual kind={item.visual}/></a><div className="card-body"><div className="card-meta"><span>{item.type.toUpperCase()} <span className="meta-dot">/</span> {item.year}</span><span className={`status ${item.status.toLowerCase()}`}>{item.status}</span></div><h3><a href={pathFor(item)} onClick={e=>openItem(e,item)}>{item.title}<ArrowUpRight size={21}/></a></h3><p>{item.summary}</p><div className="card-bottom"><span>{item.theme}</span><button onClick={e=>openItem(e,item)} aria-label={`Open ${item.title}`}><ArrowRight size={18}/></button></div></div></article>)}</div>
    ...
    <footer className="container footer" inert={selected?true:undefined}><div><span className="brand footer-brand">ro<span>.</span></span><span>Ronald Obal · Research & M&E</span></div><span>Thoughtful evidence. Meaningful progress.</span><a href="#main">Back to top<ArrowUpRight size={14}/></a></footer>
